...
public class Optimizer {
	...
	public void optimize() {
		...
		System.out.printf("there are %d (%.2f%%) out of total %d groups with no transforms in them\n", trEmpty,
				100d * trEmpty / groupsTotal, groupsTotal);
	}
	...
	private void optimizeFaces() {
		...
		for (MeshView meshView : meshViews) {
			...
			for (int i = 0; i < faces.size(); i += faceElementSize) {
				...
				final float DEAD_FACE = 1.f / 1024 / 1024 / 1024 / 1024; // taken
																			// from
																			// MeshNormal
																			// code
		...
		System.out.printf(
				"Removed %d (%.2f%%) faces with same point indexes, " + "%d (%.2f%%) faces with same points, "
						+ "%d (%.2f%%) faces with small area. " + "Total %d (%.2f%%) bad faces out of %d total.\n",
				sameIndexes, 100d * sameIndexes / total, samePoints, 100d * samePoints / total, smallArea,
				100d * smallArea / total, badTotal, 100d * badTotal / total, total);
	}
	...
	private void optimizePoints() {
		...
		for (MeshView meshView : meshViews) {
			...
			// System.out.printf("There are %d (%.2f%%) duplicate points out of
			// %d total for mesh '%s'.\n",
	...
	private void optimizeTexCoords() {
		...
		for (MeshView meshView : meshViews) {
			...
			// System.out.printf("There are %d (%.2f%%) duplicate texcoords out
			// of %d total for mesh '%s'.\n",
			// d, 100d * d / os, os, meshView.getId());
	...
	private static class MapOfLists<K, V> extends HashMap<K, List<V>> {
		...
		private static final long serialVersionUID = -4568349503529878803L;
	...
	private void parseTimeline() {
		...
		// cleanUpRepeatingFramesAndValues(); // we don't need it usually as
		// timeline is initially correct
		...
		for (KeyFrame keyFrame : sortedKeyFrames) {
			for (KeyValue keyValue : keyFrame.getValues()) {
				...
				if (prev != null && prev.keyValue.getEndValue().equals(keyValue.getEndValue())) {
					// if (prev != null && (prev.keyValue.equals(keyValue) ||
					// (prev.first &&
					// prev.keyValue.getEndValue().equals(keyValue.getEndValue()))))
					// {
					...
					if ((prevPrev != null && prevPrev.keyValue.getEndValue().equals(keyValue.getEndValue()))
						...
						// All prevPrev, prev and current match, so prev can be
						// removed
						// or prev is first and its value equals to the property
						// existing value, so prev can be removed
						toRemove.add(prev.keyFrame, prev.keyValue);
					} else {
						prevPrevValues.put(target, prev);
						// KeyInfo oldKeyInfo = prevPrevValues.put(target,
						// prev);
						// if (oldKeyInfo != null &&
						// oldKeyInfo.keyFrame.getTime().equals(prev.keyFrame.getTime()))
						// {
						// System.err.println("prevPrev replaced more than once
						// per keyFrame on " + target + "\n"
						// + "old = " + oldKeyInfo.keyFrame.getTime() + ", " +
						// oldKeyInfo.keyValue + "\n"
						// + "new = " + prev.keyFrame.getTime() + ", " +
						// prev.keyValue
						// );
						// }
					}
		...
		for (int i = 0; i < timeline.getKeyFrames().size(); i++) {
			...
			if (keyValuesToRemove != null) {
				...
				for (KeyValue keyValue : keyFrame.getValues()) {
					if (keyValuesToRemove.remove(keyValue)) {
						...
						if (convertToDiscrete) {
							newKeyValues.add(new KeyValue((WritableValue<Object>) keyValue.getTarget(),
									keyValue.getEndValue(), Interpolator.DISCRETE));
						} else {
				...
				newKeyValues.clear();
				for (KeyValue keyValue : keyFrame.getValues()) {
					newKeyValues.add(new KeyValue((WritableValue<Object>) keyValue.getTarget(), keyValue.getEndValue(),
			...
			if (keyValuesToRemove != null || convertToDiscrete) {
				if (newKeyValues.isEmpty()) {
					...
					keyFrame = new KeyFrame(keyFrame.getTime(), keyFrame.getName(), keyFrame.getOnFinished(),
							newKeyValues);
		...
		System.out.printf("Removed %d (%.2f%%) repeating KeyValues out of total %d.\n", kvRemoved,
				100d * kvRemoved / kvTotal, kvTotal);
		System.out.printf(
				"Removed %d (%.2f%%) and simplified %d (%.2f%%) KeyFrames out of total %d. %d (%.2f%%) were not removed due to event handler attached.\n",
				kfRemoved, 100d * kfRemoved / kfTotal, kfSimplified, 100d * kfSimplified / kfTotal, kfTotal,
				kfNotRemoved, 100d * kfNotRemoved / kfTotal);
		int check = 0;
	...
	private static class KeyFrameComparator implements Comparator<KeyFrame> {
		...
		public int compare(KeyFrame o1, KeyFrame o2) {
			...
			// System.err.println("those two KeyFrames are equal: o1 = " +
			// o1.getTime() + " and o2 = " + o2.getTime());
			// }
